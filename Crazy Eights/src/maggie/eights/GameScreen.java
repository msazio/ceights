...
public class GameScreen extends Activity implements View.OnClickListener{
	...
	/**
	 * Setup game after screen has been rotated - Android only
	 * */
	public void setupFromLoad(Object data){
        ...
        for(int i = 0; i < player0.size(); i++){
            tmpI = (Integer)player0.elementAt(i);
            tmpC = (Card)deck.elementAt(tmpI.intValue());
            removeCards.addElement((Card)deck.elementAt(tmpI.intValue()));
            tmpC.setFocusable(true);
            tmpC.setEnabled(true);
            tmpC.flipCardUp();
            tmpC.setOnClickListener(this);
            ((Player)players.elementAt(0)).drawCard(tmpC);
        ...
        for(int i = 0; i < player1.size(); i++){
            tmpI = (Integer)player1.elementAt(i);
            tmpC = (Card)deck.elementAt(tmpI.intValue());
            removeCards.addElement((Card)deck.elementAt(tmpI.intValue()));
            tmpC.setFocusable(false);
            tmpC.setEnabled(false);
            tmpC.flipCardDown(); 
            ((Player)players.elementAt(1)).drawCard(tmpC);
            topCards.addView(tmpC);
        }
        
        //setup discard pile
        for(int i = 0; i < discardList.size(); i++){
            tmpI = (Integer)((discardList).elementAt(i));
            discardPile.addElement((Card)deck.elementAt(tmpI.intValue()));
            removeCards.addElement((Card)deck.elementAt(tmpI.intValue()));
        }
        
        for(int i=0; i<removeCards.size(); i++)
        {
            deck.removeElement((Card)removeCards.elementAt(i));
        }
        
        tmpC = (Card)discardPile.lastElement();
        tmpC.flipCardUp();
        stackTop = tmpC;
	...
	@Override
	/**
	 * Make sure everything gets saved when screen is rotated - Android only
	 *
	 */
	public Object onRetainNonConfigurationInstance() {
	    final Object data = collectEverything();
	    return data;
	}
	
	/**
	 * Gather all info that needs to be saved during screen rotation
	 * @return data to be saved
	 */
	public Object collectEverything(){
    	LinearLayout bottomCards = (LinearLayout)findViewById(R.id.bottomCards);
        LinearLayout middleCards = (LinearLayout)findViewById(R.id.middleCards);
        LinearLayout topCards = (LinearLayout)findViewById(R.id.topCards);
        bottomCards.removeAllViews();
        middleCards.removeAllViews();
        ...
        /*Create a list of the host player's cards to be saved*/
        for(int i=0; i<((Player)players.elementAt(0)).hand.size();i++)
        {
            cardList0.addElement(new Integer(((Card)((Player)players.elementAt(0)).hand.elementAt(i)).place));
        }
        everything.addElement(cardList0);
        
        /*Create a list of the AI player's cards to be saved*/
        for(int i=0; i<((Player)players.elementAt(1)).hand.size();i++)
        {
            cardList1.addElement(new Integer(((Card)((Player)players.elementAt(1)).hand.elementAt(i)).place));
        }
        everything.addElement(cardList1);
        
        /*Create a list of the discard pile to be saved*/
        for(int i=0; i < discardPile.size(); i++)
        {
            discardList.addElement(new Integer(((Card)discardPile.elementAt(i)).place));
        }	
        everything.addElement(discardList);
